...
class Cell:
    ...
    def __sub__(self, other):
        result = self._count - other.count
        if result < 0:
            raise Exception("Ошибка: разность количества ячеек двух клеток меньше нуля")
        return Cell(result)
    ...
    def make_order(cell, count):
        array = ["*\n" if c % count == 0 else "*" for c in range(1, cell.count + 1)]
        return ''.join(array)
